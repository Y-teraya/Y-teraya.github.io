...
\begin{boxnote}
  ...
  　主に比率や成長率，増加率など\textbf{比率や割合で変化する数の平均}を算出したいときに用いる．
  ...
  　主に平均速度や並列接続時の抵抗，直列接続時のコンデンサなど\textbf{比率で表示される数の平均}を算出したいときに用いる．
...
\begin{equation}
  \sigma^2 = \frac{1}{n} \sum_{k=1}^n (x_k - \bar{x})^2= \dfrac{(x_1 - \bar{x})^2 + (x_2 - \bar{x})^2 + \cdots + (x_n - \bar{x})^2}{n}
\end{equation}
