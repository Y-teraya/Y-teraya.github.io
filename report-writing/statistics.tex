...
\label{col: basic def}
  ...
  　物体の静止質量を$m$，光子の周波数を$\nu$，プランク定数を$h$とすると，
  アインシュタインの相対性理論によれば\cref{eq: einstein}である．
...
\begin{table}[htbp]
  ...
  \begin{tabular}{rlrl}
    ...
    角度 & 1\ [deg] := $\tfrac{\pi}{180}$\ [rad] & 体積と密度 & 1\ [L] := 1\ [dm$^3$] = 0.001\ [m$^3$] \\
    パスカル & 1\ [Pa] := 1\ [kg/m \cdot s$^2$] & モル濃度 & 1\ [M] := 1\ [mol/L] \\
    ジュール & 1\ [J] := 1\ [N \cdot m] & 水素イオン濃度 & $\mathrm{pH} := - \log[\mathrm{H}^+]$ \\
...
\begin{itemize}
  \item モル計算，濃度計算: \ref{sec: mol-cal}
\end{itemize}
...
底を$a$，指数を$m, n$としたとき，\cref{eq: rpt-1,eq: rpt-2}の2法則が成立する．
...
\cref{eq: rpt-1}が正しいと仮定すると，0乗とマイナス乗を定義できる．まず，\cref{eq: rpt-1}を$n=0$とすると\cref{eq: zrp-1}である．
...
$a^m$に$a^0$を乗算しても\textbf{変化しない}．その数は，{\color{cyan}\textbf{1}}なので\cref{eq: zrp-2}と定義される．
...
\cref{eq: zrp-2}が正しいと仮定し，\cref{eq: rpt-1}を$n=-m$とすると\cref{eq: mnp-1}である．
...
\footnote{任意の数に\textbf{乗算すると1になる数}のこと．}なので\cref{eq: mnp-2}と定義される．
...
$a$を\textbf{底}，$n$を\textbf{指数}と\cref{eq: ipr-1}である．
...
辺々\footnote{両辺と同じ意味．左辺（左側の式）と右辺（右側の式）の両方の式という意味である．}$n$乗すると，\cref{eq: ipr-2}である．

\begin{equation}
  \big(\sqrt[n]{a}\big)^n = \big(a^x\big)^n \label{eq: ipr-2}
\end{equation}

左辺はルートが外れ，指数で表現される．また，分かりやすいように1乗として書くと\cref{eq: ipr-3}である．
...
指数だけ取り出して等式にすると，\cref{eq: ipr-4}である．

\begin{equation}
  1 = nx \label{eq: ipr-4}
\end{equation}

$x$について解き，$n$乗根を指数で表すと\cref{eq: ipr-5}で定義される．
...
\idx{平均値}を $\overline{x}$\footnote{$\mu$と置くこともある．}，データ数を$n$，各データを$x_k$とすると，\cref{eq: ave-1}と定義する
...
\label{col: average}
  ...
  \idx{平均値}を $G$，データ数を$n$，各データを$x_k$とすると，\cref{eq: ave-2}である
  ...
  \idx{平均値}を $H$，データ数を$n$，各データを$x_k$とすると，\cref{eq: ave-3}である．
...
データを\textbf{大きさの順に}並べた際の，真ん中の値を指す．例えば，データの集合$A$を\cref{eq: med-1}と設定する．
...
データの集合内で，同じデータが存在する場合がある．その同じデータが最も出現する値を指す．例えば，集合$A$，\cref{eq: med-1}に\cref{eq: mod-1}と設定する．
...
各データと\textbf{平均との差}の平均である．一般にデータの\textbf{バラつき}を表す．\idx{分散}を$s^2$\footnote{$\sigma^2$と置くこともある．}，データ数を$n$，各データを$x_k$とすると，\cref{eq: var-1}である．

\begin{equation}
  s^2 := \frac{1}{n} \sum_{k=1}^n (x_k - \overline{x})^2= \dfrac{(x_1 - \overline{x})^2 + (x_2 - \overline{x})^2 + \cdots + (x_n - \overline{x})^2}{n} \label{eq: var-1}
\end{equation}

\idx{標準偏差}はこの2乗を外したもののことである．すなわち，\idx{分散}$s^2$をルート$\sqrt{\quad}$に入れたものである．そのため，\idx{標準偏差}を$s$\footnote{$\sigma$と置くこともある．}とすると\cref{eq: sdv-1}である．
...
\label{col: error}
  ...
  　平均との差であれば，絶対値を用いれば良いと考えられる．絶対値で計算する平均偏差と呼ばれるものを紹介する．平均偏差は\cref{eq: err-1}で求められる．
...
単位ベクトルと係数倍を用いて，一般にベクトルを\cref{eq: cpf-1}のような式で表せる．
