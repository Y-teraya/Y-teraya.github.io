...
\subsection{間違えやすいが変換できる単位}

ここから先の内容は，知っているか知らないかの0と1である．そのため，簡単に表にまとめる程度にする．
まずは，一見では変換できなさそうだけど定義上同じものである\footnote{通常の$=$は同じことを示しているが，$:=$は\textbf{左辺を右辺と定義する}という意味である．}．

\begin{table}[htbp]
  \caption{間違えやすいが変換できる単位}
  \label{tab: unit-1}
  \centering
...
\subsection{molを含む組立単位}

やっと本題の話．molを含んだ組立単位を\cref{tab: unit-2}に示す．
...
\begin{table}[htbp]
  ...
  \begin{tabular}{rlrlrl}
    ...
    物質量と気体の体積 & 1\ [mol] &:=& 22.4\ [L] \\
    物質量と液体・個体の体積 & 1\ [mol] &:=& $\tfrac{nm}{\rho}$\tablefootnote{$n$を物質量，$m$をモル質量，$\rho$を密度とする．} \\
    \hline
  \end{tabular}
\end{table}

モル質量は大きく3つある．\textbf{原子量}，\textbf{分子量}，\textbf{式量}である．この3つは\textbf{無単位量}として扱われている．
